...
class Cone(
  ...
  fun routine(): Command {
    ...
    return if (farSide) {
      routine.fullAuto(AutoUtil.correctTrajectoryListForAlliance(Paths.WALLCONE) { RobotConstants.ALLIANCE_COLOR == DriverStation.Alliance.Red })
    } else {
      routine.fullAuto(Paths.WALLCONE)
    }
  }
}
